...
<?php if (!empty($post->post_password) && $_COOKIE['wp-postpass_' . COOKIEHASH] != $post->post_password) : ?>
	<div class="errorbox">
		<?php _e('Enter your password to view comments.', 'Julie'); ?>
	</div>
<?php return; endif; ?>
...
?>
		<div id="commentnavi">
			<span class="pages"><?php _e('Comment pages', 'Julie'); ?></span>
			<div id="commentpager">
...
?>
	...
	<?php if (pings_open()) : ?>
		<ol id="thetrackbacks">
			<?php if ($trackbacks) : $trackbackcount = 0; ?>
				<?php foreach ($trackbacks as $comment) : ?>
					<li class="trackback">
						...
						<div class="act">
							<?php edit_comment_link(__('Edit', 'Julie'), '', ''); ?>
						</div>
						<div class="fixed"></div>
						<div class="title">
			...
			<?php else : ?>
				<li class="messagebox">
					<?php _e('No trackbacks yet.', 'Julie'); ?>
				</li>
...
<?php if (!comments_open()) : // If comments are closed. ?>
	<div class="messagebox">
		<?php _e('Comments are closed.', 'Julie'); ?>
	</div>
<?php elseif ( get_option('comment_registration') && !$user_ID ) : // If registration required and not logged in. ?>
	<div id="comment_login" class="messagebox">
		...
		?>
		<?php printf(__('You must be <a href="%s">logged in</a> to post a comment.', 'Julie'), $login_link); ?>
	</div>
...
<?php else : ?>
	...
	<div id="respond">
		...
		<?php if ($user_ID) : ?>
			...
			?>
			<div class="row">
				<?php _e('Logged in as', 'Julie'); ?> <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><strong><?php echo $user_identity; ?></strong></a>.
				 <a href="<?php echo $logout_link; ?>" title="<?php _e('Log out of this account', 'Julie'); ?>"><?php _e('Logout &raquo;', 'Julie'); ?></a>
			</div>

			<?php else : ?>
			<?php if ( $comment_author != "" ) : ?>
				<div class="row">
					<?php printf(__('Welcome back <strong>%s</strong>.', 'Julie'), $comment_author) ?>
					<span id="show_author_info"><a href="javascript:void(0);" onclick="MGJS.setStyleDisplay('author_info','');MGJS.setStyleDisplay('show_author_info','none');MGJS.setStyleDisplay('hide_author_info','');"><?php _e('Change &raquo;', 'Julie'); ?></a></span>
					<span id="hide_author_info"><a href="javascript:void(0);" onclick="MGJS.setStyleDisplay('author_info','none');MGJS.setStyleDisplay('show_author_info','');MGJS.setStyleDisplay('hide_author_info','none');"><?php _e('Close &raquo;', 'Julie'); ?></a></span>
				</div>
			<?php endif; ?>
			...
			<div id="author_info">
				<div class="row">
					...
					<label for="author" class="small"><?php _e('Name', 'Julie'); ?> <?php if ($req) _e('(required)', 'Julie'); ?></label>
				</div>
				<div class="row">
					<input type="text" name="email" id="email" class="textfield" value="<?php echo $comment_author_email; ?>" size="24" tabindex="2" />
					<label for="email" class="small"><?php _e('E-Mail (will not be published)', 'Julie');?> <?php if ($req) _e('(required)', 'Julie'); ?></label>
				</div>
				<div class="row">
					<input type="text" name="url" id="url" class="textfield" value="<?php echo $comment_author_url; ?>" size="24" tabindex="3" />
					<label for="url" class="small"><?php _e('Website', 'Julie'); ?></label>
				</div>
			</div>
		...
		<div id="submitbox">
			<div class="submitbutton">
				<input name="submit" type="submit" id="submit" class="button" tabindex="5" value="<?php _e('Submit Comment', 'Julie'); ?>" />
			</div>
