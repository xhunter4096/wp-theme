...
class JulieOptions {
	...
	function getOptions() {
		...
		if (!is_array($options)) {
			...
			update_option('Julie_options', $options);
		}
		return $options;
	}

	function add() {
		if(isset($_POST['Julie_save'])) {
			$options = JulieOptions::getOptions();
			...
			JulieOptions::getOptions();
		}

		add_theme_page(__('Current Theme Options', 'Julie'), __('Current Theme Options', 'Julie'), 'edit_themes', basename(__FILE__), array('JulieOptions', 'display'));
	}
...
<form action="#" method="post" enctype="multipart/form-data" name="Julie_form" id="Julie_form">
	<div class="wrap">
		<h2><?php _e('Current Theme Options', 'Julie'); ?></h2>

		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th scope="row"><?php _e('Search', 'Julie'); ?></th>
					<td>
						<label>
							<input name="google_cse" type="checkbox" value="checkbox" <?php if($options['google_cse']) echo "checked='checked'"; ?> />
							 <?php _e('Using google custom search engine.', 'Julie'); ?>
						</label>
						<br/>
						<?php _e('CX:', 'Julie'); ?>
						 <input type="text" name="google_cse_cx" id="google_cse_cx" class="code" size="40" value="<?php echo($options['google_cse_cx']); ?>">
						<br/>
						<?php printf(__('Find <code>name="cx"</code> in the <strong>Search box code</strong> of <a href="%1$s">Google Custom Search Engine</a>, and type the <code>value</code> here.<br/>For example: <code>014782006753236413342:1ltfrybsbz4</code>', 'Julie'), 'http://www.google.com/coop/cse/'); ?>
					</td>
				</tr>
			</tbody>
		</table>
			...
			<tbody>
				<tr valign="top">
					<th scope="row">
						<?php _e('Notice', 'Julie'); ?>
						<br/>
						<small style="font-weight:normal;"><?php _e('HTML enabled', 'Julie'); ?></small>
					</th>
					<td>
						<!-- notice START -->
						<label>
							<input name="notice" type="checkbox" value="checkbox" <?php if($options['notice']) echo "checked='checked'"; ?> />
							 <?php _e('This notice bar will display at the top of posts on homepage.', 'Julie'); ?>
						</label>
						<br />
						<label>
		...
		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th scope="row">
						<?php _e('Showcase', 'Julie'); ?>
						<br/>
						<small style="font-weight:normal;"><?php _e('HTML enabled', 'Julie'); ?></small>
					</th>
					<td>
						<!-- showcase START -->
						<?php _e('This showcase will display at the top of sidebar.', 'Julie'); ?>
						<br/>
						<?php _e('Who can see?', 'Julie'); ?>
						<label style="margin-left:10px;">
		...
		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th scope="row"><?php _e('Feed', 'Julie'); ?></th>
					<td>
						<label>
							<input name="feed_readers" type="checkbox" value="checkbox" <?php if($options['feed_readers']) echo "checked='checked'"; ?> />
							 <?php _e('Show the feed reader list when mouse over on feed button.', 'Julie'); ?>
						</label>
						<br />
						<label>
							<input name="feed" type="checkbox" value="checkbox" <?php if($options['feed']) echo "checked='checked'"; ?> />
							 <?php _e('Custom feed.', 'Julie'); ?>
						</label>
						 <?php _e('URL:', 'Julie'); ?> <input type="text" name="feed_url" id="feed_url" class="code" size="60" value="<?php echo($options['feed_url']); ?>">
						<br/>
						<label>
							<input name="feed_email" type="checkbox" value="checkbox" <?php if($options['feed_email']) echo "checked='checked'"; ?> />
							 <?php _e('Email feed.', 'Julie'); ?>
						</label>
						 <?php _e('URL:', 'Julie'); ?> <input type="text" name="feed_url_email" id="feed_url_email" class="code" size="60" value="<?php echo($options['feed_url_email']); ?>">
					</td>
				</tr>
			</tbody>
		</table>
		...
		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th scope="row"><?php _e('Twitter', 'Julie'); ?></th>
					<td>
						<label>
							<input name="twitter" type="checkbox" value="checkbox" <?php if($options['twitter']) echo "checked='checked'"; ?> />
							 <?php _e('Add Twitter button.', 'Julie'); ?>
						</label>
						<br />
						 <?php _e('Twitter username:', 'Julie'); ?>
					...
					<td>
						<label>
							<input name="analytics" type="checkbox" value="checkbox" <?php if($options['analytics']) echo "checked='checked'"; ?> />
							 <?php _e('Add web analytics code to your site. (e.g. Google Analytics, Yahoo! Web Analytics, ...)', 'Julie'); ?>
						</label>
						<label>
		...
		<p class="submit">
			<input class="button-primary" type="submit" name="Julie_save" value="<?php _e('Save Changes', 'Julie'); ?>" />
		</p>
	</div>
</form>
...
// register functions
add_action('admin_menu', array('JulieOptions', 'add'));


/** l10n */
function theme_init(){
	load_theme_textdomain('Julie', get_template_directory() . '/languages');
}
...
?>
	<li class="comment <?php if($comment->comment_author_email == get_the_author_email()) {echo 'admincomment';} else {echo 'regularcomment';} ?>" id="comment-<?php comment_ID() ?>">
		...
		<div class="info">
			<div class="date">
				<?php printf( __('%1$s at %2$s', 'Julie'), get_comment_time(__('F jS, Y', 'Julie')), get_comment_time(__('H:i', 'Julie')) ); ?>
					 | <a href="#comment-<?php comment_ID() ?>"><?php printf('#%1$s', ++$commentcount); ?></a>
			</div>
			<div class="act">
				<a href="javascript:void(0);" onclick="MGJS_CMT.reply('commentauthor-<?php comment_ID() ?>', 'comment-<?php comment_ID() ?>', 'comment');"><?php _e('Reply', 'Julie'); ?></a> | 
				<a href="javascript:void(0);" onclick="MGJS_CMT.quote('commentauthor-<?php comment_ID() ?>', 'comment-<?php comment_ID() ?>', 'commentbody-<?php comment_ID() ?>', 'comment');"><?php _e('Quote', 'Julie'); ?></a>
				<?php
					if (function_exists("qc_comment_edit_link")) {
						qc_comment_edit_link('', ' | ', '', __('Edit', 'Julie'));
					}
					edit_comment_link(__('Advanced edit', 'Julie'), ' | ', '');
				?>
			</div>
			<div class="fixed"></div>
			<div class="content">
				<?php if ($comment->comment_approved == '0') : ?>
					<p><small><?php _e('Your comment is awaiting moderation.', 'Julie'); ?></small></p>
				<?php endif; ?>
